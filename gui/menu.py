...
LEVEL_DETAILS = {
    ...
    5: (
        "Level 6: Minimax / Alpha-Beta",
        "Đối kháng Minimax & Alpha-Beta (Deep Search).",
    ),
}
...
class StartMenu:
    def __init__(self, width, height):
        ...
        # Load fonts
        self.brand_font = pygame.font.SysFont(
            "Playfair Display, Segoe UI, Tahoma", 46, bold=True
        )
        self.brand_sub_font = pygame.font.SysFont(
            "Source Serif 4, Segoe UI, Tahoma", 18
        )
        ...
        self.seal_cy = 615
        self.seal_r = 55
        self.btn_seal = pygame.Rect(
            self.seal_cx - self.seal_r,
            self.seal_cy - self.seal_r,
            self.seal_r * 2,
            self.seal_r * 2,
        )
    ...
    def handle_event(self, event):
        ...
        if self.state == "mode_select":
            ...
            if btn_human.collidepoint(mouse_pos):
                btn_human.x += 8
            ...
            if btn_online.collidepoint(mouse_pos):
                btn_online.x += 8

            btn_shop = self.btn_mode_shop.copy()
            if btn_shop.collidepoint(mouse_pos):
                btn_shop.x += 8
    ...
    def draw(self, surface):
        ...
        if self.state == "mode_select":
            ...
            sub_lbl = self.brand_sub_font.render(
                "Đỉnh Cao Trí Tuệ • Khí Chất Hoàng Gia", True, COLOR_TEXT_MUTED
            )
            ...
            dec_y = 375
            pygame.draw.line(
                surface, COLOR_OUTLINE, (title_x, dec_y), (title_x + 320, dec_y), 1
            )
            pygame.draw.polygon(
                surface,
                COLOR_GOLD,
            ...
            footer_txt = self.footer_font.render(
                "Nhóm 1 - HCMUTE", True, (120, 110, 100)
            )
            ...
            for rect, text, icon in buttons:
                ...
                pygame.draw.rect(
                    surface,
                    COLOR_CARD_BG if is_hover else (35, 20, 16),
                    draw_rect,
                    0,
                    8,
                )
                pygame.draw.rect(
                    surface,
                    COLOR_GOLD if is_hover else COLOR_OUTLINE,
                    draw_rect,
                ...
                surface.blit(
                    b_txt,
                    (draw_rect.x + 20, draw_rect.centery - b_txt.get_height() // 2),
                )
                ...
                surface.blit(
                    i_txt,
                    (draw_rect.right - 38, draw_rect.centery - i_txt.get_height() // 2),
                )
        ...
        # --- STATE: algo_select_red / algo_select_black ---
            ...
            pygame.draw.rect(
                surface,
                (60, 40, 30) if is_hover_back else (35, 20, 16),
                self.btn_level_back,
                0,
                6,
            )
            ...
            surface.blit(
                back_txt,
                (
                    self.btn_level_back.centerx - back_txt.get_width() // 2,
                    self.btn_level_back.centery - back_txt.get_height() // 2,
                ),
            )
            ...
            surface.blit(
                hdr_shadow, (self.width // 2 - hdr_shadow.get_width() // 2 + 2, 62)
            )
            ...
            sub_lbl = self.brand_sub_font.render(
                f"{level_title} — {side_text}", True, COLOR_TEXT_MUTED
            )
            ...
            # Instruction text
            instr = self.body_font.render(
                "Chọn một thuật toán bên dưới:", True, COLOR_TEXT
            )
            ...
            for i, rect in enumerate(self.algo_btn_rects):
                ...
                surface.blit(
                    a_txt,
                    (
                        rect.centerx - a_txt.get_width() // 2,
        ...
        # --- STATE: level_red / level_black (Cấu Hình Trí Tuệ) ---
            ...
            active_side_text = (
                "BÊN ĐỎ (RED)" if self.state == "level_red" else "BÊN ĐEN (BLACK)"
            )
            active_side_color = (
                COLOR_RED_TEXT if self.state == "level_red" else COLOR_BLACK_TEXT
            )
            ...
            sub_lbl = self.brand_sub_font.render(
                f"Lựa chọn cấp độ thuật toán đối kháng cho {active_side_text}",
                True,
                COLOR_TEXT_MUTED,
            )
            surface.blit(sub_lbl, (self.width // 2 - sub_lbl.get_width() // 2, 125))
            ...
            for i, rect in enumerate(self.level_rects):
                ...
                card_alpha = (
                    100
                    if is_dimmed
                    else (235 if is_selected else (190 if is_hover else 150))
                )
                ...
                border_color = (
                    COLOR_GOLD
                    if is_selected
                    else (COLOR_GOLD if is_hover else COLOR_OUTLINE)
                )
                ...
                title_col = (
                    COLOR_GOLD
                    if is_selected
                    else (COLOR_TEXT if not is_dimmed else COLOR_TEXT_MUTED)
                )
                ...
                t_lbl = self.body_font.render(
                    algo_title.split(":")[-1].strip(), True, title_col
                )
            ...
            is_hover_seal = (
                self.btn_seal.collidepoint(mouse_pos) and selected_level is not None
            )
            ...
            for r_val in range(r_outer, 0, -2):
                ...
                cr = int(
                    seal_color_inner[0]
                    + (seal_color_outer[0] - seal_color_inner[0]) * ratio
                )
                cg = int(
                    seal_color_inner[1]
                    + (seal_color_outer[1] - seal_color_inner[1]) * ratio
                )
                cb = int(
                    seal_color_inner[2]
                    + (seal_color_outer[2] - seal_color_inner[2]) * ratio
                )
                pygame.draw.circle(
                    surface, (cr, cg, cb), (self.seal_cx, self.seal_cy), r_val
                )

            # Seal border
            seal_border_color = COLOR_GOLD if seal_active else (150, 150, 150)
            pygame.draw.circle(
                surface, seal_border_color, (self.seal_cx, self.seal_cy), r_outer, 2
            )

            # Text inside seal
            txt_start = self.body_font.render(
                "BẮT ĐẦU", True, COLOR_GOLD if seal_active else (180, 180, 180)
            )
            txt_khai = self.detail_font.render(
                "Khai Chiến", True, (255, 193, 7) if seal_active else (160, 160, 160)
            )

            surface.blit(
                txt_start,
                (self.seal_cx - txt_start.get_width() // 2, self.seal_cy - 12),
            )
            surface.blit(
                txt_khai, (self.seal_cx - txt_khai.get_width() // 2, self.seal_cy + 10)
            )
        ...
        if time.time() < self.popup_timer:
            ...
            surface.blit(
                p_txt,
                (
                    self.width // 2 - p_txt.get_width() // 2,
                    self.height // 2 - p_txt.get_height() // 2,
                ),
            )
