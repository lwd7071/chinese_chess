...
class Sidebar:
    ...
    def update_layout(self, x, y, width, height):
        ...
        self.history_view_rect = pygame.Rect(
            x + 15, 545, width - 30, self.history_card_height - 95
        )
    ...
    def handle_event(self, event, game_mode="human_vs_bot"):
        ...
        dropdown_rect_red = pygame.Rect(
            self.dropdown_rect.x, self.dropdown_rect.y, rect_w, 28
        )
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.dropdown_open:
                ...
                if dropdown_list_rect.collidepoint(event.pos):
                    ...
                    clicked_idx = self.dropdown_scroll + (
                        relative_y // self.dropdown_item_height
                    )
                    ...
                    if game_mode == "human_vs_bot":
                        ...
                        if not dropdown_rect_red.collidepoint(
                            event.pos
                        ) and not dropdown_rect_black.collidepoint(event.pos):
                            return None
            ...
            if game_mode == "human_vs_bot":
                ...
                if dropdown_rect_red.collidepoint(event.pos):
                    ...
                    self.dropdown_open = (
                        "black" if self.dropdown_open != "black" else False
                    )
                    self.dropdown_scroll = 0
                    from gui.sound import play_synth_sound
            ...
            if self.history_view_rect.collidepoint(pos):
                ...
                self.history_scroll = max(
                    0, min(max_scroll, self.history_scroll - event.y)
                )
                return None
            ...
            if self.dropdown_open:
                ...
                if dropdown_list_rect.collidepoint(pos):
                    max_dropdown_scroll = max(
                        0, len(ALGO_OPTIONS) - self.dropdown_visible_items
                    )
                    self.dropdown_scroll = max(
                        0, min(max_dropdown_scroll, self.dropdown_scroll - event.y)
                    )
                    return None
    ...
    def draw(
        ...
        pygame.draw.line(
            ...
            COLOR_OUTLINE,
            (self.x + 15, self.y + 55),
            (self.x + self.width - 15, self.y + 55),
            1,
        )
        ...
        if opp_img:
            ...
            surface.blit(
                opp_txt,
                (
                    opp_av_center[0] - opp_txt.get_width() // 2,
                    opp_av_center[1] - opp_txt.get_height() // 2,
                ),
            )
        ...
        opp_name_str = (
            black_bot_name
            if game_mode == "human_vs_bot"
            else black_bot_name.split(":")[-1].strip()
        )
        ...
        surface.blit(
            black_exp_txt, (self.x + self.width - 68 - black_exp_txt.get_width(), 108)
        )
        ...
        turn_val_str = (
            ...
            else ("LƯỢT CỦA ĐỎ" if board.turn == "red" else "LƯỢT CỦA ĐEN")
        )
        ...
        if play_img:
            ...
            surface.blit(
                play_txt,
                (
                    play_av_center[0] - play_txt.get_width() // 2,
                    play_av_center[1] - play_txt.get_height() // 2,
                ),
            )
        ...
        play_name_str = (
            "Kỳ Vương"
            if game_mode == "human_vs_bot"
            else red_bot_name.split(":")[-1].strip()
        )
        play_name_str = play_name_str or "Kỳ Vương"
        play_name = self.body_font.render(
            play_name_str,
            True,
            COLOR_RED if game_mode != "human_vs_bot" else COLOR_ACCENT,
        )
        ...
        stats_lbl = self.small_font.render(
            "Bảng Xếp Hạng & Thống Kê", True, COLOR_ACCENT
        )
        ...
        surface.blit(
            rec_val, (stats_rect.right - rec_val.get_width() - 15, stats_rect.y + 30)
        )

        reward_lbl = self.small_font.render(
            "Phần thưởng thắng:", True, COLOR_TEXT_MUTED
        )
        reward_val = self.mono_font.render("+50 Vàng", True, COLOR_ACCENT)
        surface.blit(reward_lbl, (stats_rect.x + 15, stats_rect.y + 52))
        surface.blit(
            reward_val,
            (stats_rect.right - reward_val.get_width() - 15, stats_rect.y + 50),
        )
        ...
        algo_header = self.title_font.render(
            "BẢNG ĐIỀU KHIỂN THUẬT TOÁN", True, COLOR_ACCENT
        )
        ...
        # Red Bot
        active_algo_key_red = (
            red_bot_name.split(":")[-1].strip() if ":" in red_bot_name else red_bot_name
        )
        ...
        # Black Bot
        active_algo_key_black = (
            black_bot_name.split(":")[-1].strip()
            if ":" in black_bot_name
            else black_bot_name
        )
        ...
        if game_mode == "human_vs_bot":
            ...
            pygame.draw.rect(
                surface,
                COLOR_ACCENT if self.dropdown_open else COLOR_OUTLINE,
                self.dropdown_rect,
                1,
                4,
            )
            ...
            surface.blit(
                dd_txt,
                (
                    self.dropdown_rect.x + 10,
                    self.dropdown_rect.centery - dd_txt.get_height() // 2,
                ),
            )
            ...
            surface.blit(
                arrow_txt,
                (
                    self.dropdown_rect.right - 20,
                    self.dropdown_rect.centery - arrow_txt.get_height() // 2,
                ),
            )
        else:
            ...
            pygame.draw.rect(
                surface,
                COLOR_ACCENT if self.dropdown_open == "red" else COLOR_OUTLINE,
                dropdown_rect_red,
                1,
                4,
            )
            ...
            surface.blit(
                dd_txt_red,
                (
                    dropdown_rect_red.x + 8,
                    dropdown_rect_red.centery - dd_txt_red.get_height() // 2,
                ),
            )
            ...
            surface.blit(
                arrow_txt_red,
                (
                    dropdown_rect_red.right - 18,
                    dropdown_rect_red.centery - arrow_txt_red.get_height() // 2,
                ),
            )
            ...
            pygame.draw.rect(
                surface,
                COLOR_ACCENT if self.dropdown_open == "black" else COLOR_OUTLINE,
                dropdown_rect_black,
                1,
                4,
            )
            ...
            surface.blit(
                dd_txt_black,
                (
                    dropdown_rect_black.x + 8,
                    dropdown_rect_black.centery - dd_txt_black.get_height() // 2,
                ),
            )

            arrow_char_black = "▲" if self.dropdown_open == "black" else "▼"
            arrow_txt_black = self.tiny_font.render(
                arrow_char_black, True, COLOR_ACCENT
            )
            surface.blit(
                arrow_txt_black,
                (
                    dropdown_rect_black.right - 18,
                    dropdown_rect_black.centery - arrow_txt_black.get_height() // 2,
                ),
            )
        ...
        if current_move_count < self.last_move_count or (
            current_move_count == 0 and self.sim_nodes > 3000
        ):
        ...
        if current_move_count > self.last_move_count:
            ...
            if is_bot_move:
                ...
                self.sim_frontier = max(
                    10, self.sim_frontier + added_frontier - random.randint(5, 25)
                )
        ...
        for idx in range(3):
            ...
            surface.blit(
                v_txt, (b_rect.centerx - v_txt.get_width() // 2, b_rect.y + 20)
            )
        ...
        pygame.draw.rect(
            surface,
            (30, 20, 16),
            (algo_card_rect.x + 15, progress_y, algo_card_rect.width - 30, 6),
            0,
            3,
        )
        ...
        if is_bot_thinking:
            ...
            pulse_offset = int(
                (pygame.time.get_ticks() * 0.08) % (algo_card_rect.width - 100)
            )
            pygame.draw.rect(
                surface,
                COLOR_ACCENT,
                (algo_card_rect.x + 15 + pulse_offset, progress_y, 70, 6),
                0,
                3,
            )
            ...
            calc_lbl = self.tiny_font.render(
                "ĐANG TÍNH..." if is_pulse_alpha else "", True, COLOR_ACCENT
            )
            surface.blit(
                calc_lbl,
        ...
        hist_stats = self.tiny_font.render(
            f"Tổng: {total_moves} nước | Đỏ: {math.ceil(total_moves / 2)} | Đen: {total_moves // 2}",
            True,
            COLOR_TEXT_MUTED,
        )
        ...
        pygame.draw.line(
            surface,
            COLOR_OUTLINE,
            (history_card_rect.x + 15, hdr_y + 16),
            (history_card_rect.right - 15, hdr_y + 16),
            1,
        )
        ...
        for idx, (red_m, black_m) in enumerate(history_rows):
            y = start_y + idx * 24
            pygame.Rect(
                history_card_rect.x + 10, y - 2, history_card_rect.width - 25, 20
            )
            ...
            row_num_lbl = self.count_font.render(
                f"{start_row_idx + idx + 1}.", True, COLOR_TEXT_MUTED
            )
            ...
            red_color = (
                COLOR_ACCENT
                if is_red_latest and time.time() < latest_move_flash_until
                else COLOR_TEXT
            )
            if red_m.get("captured_char"):
                red_color = COLOR_JADE
            ...
            if black_m:
                ...
                black_color = (
                    COLOR_ACCENT
                    if is_black_latest and time.time() < latest_move_flash_until
                    else COLOR_TEXT
                )
                if black_m.get("captured_char"):
                    black_color = COLOR_JADE
        ...
        if max_rows_scroll > 0:
            ...
            pygame.draw.rect(
                surface, (50, 30, 25), (track_x, track_y, 4, track_h), 0, 2
            )

            visible_ratio = min(
                1.0, self.history_visible_rows / max(1, self._history_total)
            )
            thumb_h = max(16, int(track_h * visible_ratio))
            thumb_range = max(1, track_h - thumb_h)
            thumb_y = track_y + int(
                (thumb_range * self.history_scroll) / max_rows_scroll
            )
            pygame.draw.rect(
                surface, COLOR_ACCENT, (track_x - 1, thumb_y, 6, thumb_h), 0, 2
            )

        # Hint line at history footer
        hist_hint = self.tiny_font.render(
            "Cuộn chuột trong bảng lịch sử để xem thêm", True, COLOR_TEXT_MUTED
        )
        surface.blit(
            hist_hint, (history_card_rect.x + 15, history_card_rect.bottom - 18)
        )
        ...
        if game_mode == "bot_vs_bot":
            ...
            pygame.draw.rect(
                surface,
                (52, 152, 219) if is_hover_ret else (41, 128, 185),
                rect_ret,
                0,
                6,
            )
            ...
            surface.blit(
                lbl_txt_ret,
                (
                    rect_ret.centerx - lbl_txt_ret.get_width() // 2,
                    rect_ret.centery - lbl_txt_ret.get_height() // 2,
                ),
            )
            ...
            pygame.draw.rect(
                surface,
                pause_hover_color if is_hover_pause else pause_color,
                rect_pause,
                0,
                6,
            )
            ...
            surface.blit(
                lbl_txt_pause,
                (
                    rect_pause.centerx - lbl_txt_pause.get_width() // 2,
                    rect_pause.centery - lbl_txt_pause.get_height() // 2,
                ),
            )
        else:
            ...
            for rect, label, hover_color, normal_color in buttons:
                ...
                pygame.draw.rect(
                    surface, hover_color if is_hover else normal_color, rect, 0, 6
                )
                ...
                surface.blit(
                    lbl_txt,
                    (
                        rect.centerx - lbl_txt.get_width() // 2,
                        rect.centery - lbl_txt.get_height() // 2,
                    ),
                )
        ...
        if self.dropdown_open:
            ...
            for i in range(self.dropdown_visible_items):
                ...
                item_rect = pygame.Rect(
                    dropdown_list_rect.x + 2,
                    iy,
                    dropdown_list_rect.width - 4,
                ...
                item_color = (
                    ...
                    else (COLOR_TEXT if is_item_hover else COLOR_TEXT_MUTED)
                )
                ...
                surface.blit(
                    prefix_lbl,
                    (item_rect.x + 6, item_rect.centery - prefix_lbl.get_height() // 2),
                )
                ...
                if name_lbl.get_width() > max_text_w:
                    ...
                    while len(truncated_name) > 3 and name_lbl.get_width() > max_text_w:
                        ...
                        name_lbl = self.small_font.render(
                            truncated_name + "...", True, item_color
                        )

                surface.blit(
                    name_lbl,
                    (item_rect.x + 24, item_rect.centery - name_lbl.get_height() // 2),
                )
            ...
            if max_dropdown_scroll > 0:
                ...
                pygame.draw.rect(
                    surface, (50, 30, 25), (dd_track_x, dd_track_y, 4, dd_track_h), 0, 2
                )
                ...
                dd_thumb_y = dd_track_y + int(
                    (dd_thumb_range * self.dropdown_scroll) / max_dropdown_scroll
                )
                pygame.draw.rect(
                    surface,
                    COLOR_ACCENT,
                    (dd_track_x - 1, dd_thumb_y, 6, dd_thumb_h),
                    0,
                    2,
                )
