...
class Renderer:
    def __init__(self, cell_size=60, offset_x=40, offset_y=50):
        ...
        if self.chinese_supported:
            self.piece_font = pygame.font.SysFont(
                self.font_name, int(cell_size * 0.48), bold=True
            )
            self.label_font = pygame.font.SysFont(self.font_name, int(cell_size * 0.38))
        else:
            self.piece_font = pygame.font.SysFont(
                "Segoe UI, Tahoma, Arial", int(cell_size * 0.5), bold=True
            )
            self.label_font = pygame.font.SysFont(
                "Segoe UI, Tahoma, Arial", int(cell_size * 0.3)
            )

        self.tooltip_header_font = pygame.font.SysFont(
            "Segoe UI, Tahoma, Arial", int(cell_size * 0.28), bold=True
        )
        self.tooltip_body_font = pygame.font.SysFont(
            "Segoe UI, Tahoma, Arial", int(cell_size * 0.24)
        )
    ...
    def update_layout(self, cell_size, offset_x, offset_y, board_width, board_height):
        ...
        if self.chinese_supported:
            ...
            self.label_font = pygame.font.SysFont(
                "Segoe UI, Tahoma, Arial", int(cell_size * 0.38)
            )
        ...
        for theme_name, theme_data in THEMES.items():
            ...
            if theme_name == "classic_wood":
                ...
                for y in range(0, self.board_height, 2):
                    ...
                    pygame.draw.line(
                        surf, (r, g, b), (0, y + 1), (self.board_width, y + 1)
                    )
                ...
                glow = pygame.Surface(
                    (self.board_width, self.board_height), pygame.SRCALPHA
                )
                pygame.draw.rect(
                    glow,
                    (0, 165, 230, 25),
    ...
    def draw_board(self, surface, theme="classic_wood"):
        ...
        if theme != "classic_wood":
            asset_map = {"white_marble": "board_marble", "dark_glass": "board_glass"}
            asset_name = asset_map.get(theme)
            img = get_asset(asset_name) if asset_name else None
            if img:
                scaled = pygame.transform.smoothscale(
                    img, (self.board_width, self.board_height)
                )
    ...
    def draw_piece(
        ...
        pygame.draw.circle(
            shadow_surf,
            (0, 0, 0, shadow_alpha),
            (radius + shadow_offset, radius + shadow_offset),
            radius,
        )
        surface.blit(
            shadow_surf, (cx - radius - shadow_offset, cy - radius - shadow_offset)
        )
        ...
        text_color = (
            skin_data["red_text"] if piece.color == "red" else skin_data["black_text"]
        )
        ...
        for color in ["red", "black"]:
            if board.is_in_check(color):
                ...
                if g_pos:
                    ...
                    check_font = pygame.font.SysFont(
                        "Segoe UI, Tahoma, Arial", 20, bold=True
                    )
    ...
    def cache_tooltips(self):
        for name, desc in PIECE_DESCRIPTIONS.items():
            header_surface = self.tooltip_header_font.render(
                desc["name"], True, (242, 202, 80)
            )
            ...
            box_h = (
                header_surface.get_height() + len(rule_surfaces) * 16 + padding * 2 + 8
            )
