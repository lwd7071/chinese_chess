...
class SettingsScreen:
    ...
    def load_profile(self):
        ...
        if "name" not in self.data:
            self.data["name"] = "Kỳ Vương"
        if "country" not in self.data:
            self.data["country"] = "Việt Nam"
        if "birthday" not in self.data:
            self.data["birthday"] = "01/01/2000"
    ...
    def add_match_record(self, mode, algo, result, exp_gained=None):
        ...
        if mode == "human_vs_bot":
            if exp_gained is None:
                exp_gained = (
                    100 if result == "Thắng" else (40 if result == "Hòa" else 20)
                )
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(
            surface,
            (55, 35, 30) if is_hover_back else COLOR_CARD_BG,
            self.btn_back,
            0,
            6,
        )
        ...
        surface.blit(
            lbl_back,
            (
                self.btn_back.centerx - lbl_back.get_width() // 2,
                self.btn_back.centery - lbl_back.get_height() // 2,
            ),
        )
        ...
        bg_p = (
            (60, 45, 35)
            if is_active_p
            else ((55, 35, 30) if is_hover_p else COLOR_CARD_BG)
        )
        ...
        lbl_p = self.tab_font.render(
            "HỒ SƠ", True, COLOR_GOLD if is_active_p else COLOR_TEXT
        )
        surface.blit(
            lbl_p,
            (
                self.btn_tab_profile.centerx - lbl_p.get_width() // 2,
                self.btn_tab_profile.centery - lbl_p.get_height() // 2,
            ),
        )
        ...
        bg_h = (
            (60, 45, 35)
            if is_active_h
            else ((55, 35, 30) if is_hover_h else COLOR_CARD_BG)
        )
        ...
        lbl_h = self.tab_font.render(
            "LỊCH SỬ ĐẤU", True, COLOR_GOLD if is_active_h else COLOR_TEXT
        )
        surface.blit(
            lbl_h,
            (
                self.btn_tab_history.centerx - lbl_h.get_width() // 2,
                self.btn_tab_history.centery - lbl_h.get_height() // 2,
            ),
        )
    ...
    def draw_profile_tab(self, surface):
        ...
        surface.blit(
            lvl_num,
            (
                lvl_center_x - lvl_num.get_width() // 2,
                lvl_center_y - lvl_num.get_height() // 2,
            ),
        )
        ...
        pygame.draw.rect(
            surface,
            COLOR_GOLD,
            (bar_x, bar_y, int(bar_w * progress_ratio), bar_h),
            0,
            5,
        )
        pygame.draw.rect(surface, COLOR_OUTLINE, (bar_x, bar_y, bar_w, bar_h), 1, 5)

        exp_txt = self.tiny_font.render(
            f"EXP: {current_exp}/{required_exp} ({(progress_ratio * 100):.1f}%)",
            True,
            COLOR_MUTED,
        )
        ...
        for label_text, val, rect, key in fields:
            ...
            surface.blit(
                txt_val, (rect.x + 12, rect.centery - txt_val.get_height() // 2)
            )
        ...
        # Footer Help tips
        help_lbl = self.tiny_font.render(
            "Nhấp chuột vào các ô trên để nhập và nhấn Enter để lưu lại thay đổi",
            True,
            COLOR_MUTED,
        )
        surface.blit(
            help_lbl, (center_x - help_lbl.get_width() // 2, form_rect.bottom + 12)
        )
    ...
    def draw_history_tab(self, surface):
        ...
        col_w3 = 140  # Kết quả
        table_w - col_w1 - col_w2 - col_w3 - 20  # Ngày giờ (remaining width)

        hdr_rect = pygame.Rect(42, table_y + 2, table_w - 4, 38)
        pygame.draw.rect(surface, (20, 12, 10), hdr_rect, 0, 6)
        pygame.draw.line(
            surface, COLOR_OUTLINE, (40, table_y + 40), (40 + table_w, table_y + 40), 1
        )
        ...
        surface.blit(
            lbl_h3, (60 + col_w1 + col_w2, table_y + 20 - lbl_h3.get_height() // 2)
        )
        surface.blit(
            lbl_h4,
            (60 + col_w1 + col_w2 + col_w3, table_y + 20 - lbl_h4.get_height() // 2),
        )

        # Match History data rendering
        history = self.data.get("history", [])
        if not history:
            no_txt = self.value_font.render(
                "Chưa có lịch sử đấu nào được ghi nhận", True, COLOR_MUTED
            )
        ...
        start_y = table_y + 41
        visible_count = min(
            len(history) - self.history_scroll, self.history_visible_rows
        )
        ...
        for idx in range(visible_count):
            ...
            pygame.draw.line(
                surface,
                (50, 42, 32),
                (40, y + self.row_height),
                (40 + table_w, y + self.row_height),
                1,
            )

            # Values render
            mode_txt = self.value_font.render(
                record.get("mode", "Tự động"), True, COLOR_TEXT
            )
            algo_txt = self.value_font.render(
                record.get("algo", "Tự động"), True, COLOR_TEXT
            )
            ...
            res_color = (
                COLOR_ACCENT
                if result == "Thắng"
                else (COLOR_RED if result == "Thua" else COLOR_GOLD)
            )
            res_txt = self.label_font.render(result, True, res_color)

            # Blit columns
            surface.blit(
                mode_txt, (60, y + self.row_height // 2 - mode_txt.get_height() // 2)
            )
            surface.blit(
                algo_txt,
                (60 + col_w1, y + self.row_height // 2 - algo_txt.get_height() // 2),
            )
            surface.blit(
                res_txt,
                (
                    60 + col_w1 + col_w2,
                    y + self.row_height // 2 - res_txt.get_height() // 2,
                ),
            )
            surface.blit(
                time_txt,
                (
                    60 + col_w1 + col_w2 + col_w3,
                    y + self.row_height // 2 - time_txt.get_height() // 2,
                ),
            )
        ...
        if max_rows_scroll > 0:
            ...
            pygame.draw.rect(
                surface, COLOR_GOLD, (track_x - 1, thumb_y, 6, thumb_h), 0, 2
            )

            # Bottom help tip
            scroll_tip = self.tiny_font.render(
                "Cuộn chuột trong bảng lịch sử để xem thêm", True, COLOR_MUTED
            )
            surface.blit(
                scroll_tip,
                (40 + table_w - scroll_tip.get_width() - 15, table_y + table_h - 18),
            )
