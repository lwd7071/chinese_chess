...
class TestThreadSafety(unittest.TestCase):
    ...
    def test_controller_has_ai_lock(self, *mocks):
        ...
        self.assertTrue(
            hasattr(controller, "ai_lock"),
            "GameController should have an ai_lock attribute",
        )
    ...
    def test_calculate_uses_lock(self, *mocks):
        ...
        with (
            patch("main.has_lost", return_value=False),
            ...
            self.assertIsNotNone(
                started_thread, "AI calculation thread should have been started"
            )
            started_thread.join()

            # The calculation should have entered/exited the lock to set self.ai_result
            self.assertTrue(
                mock_lock.__enter__.called,
                "Lock should be acquired during calculation result assignment",
            )
    ...
    def test_get_result_uses_lock(self, *mocks):
        ...
        self.assertTrue(
            mock_lock.__enter__.called,
            "Lock should be acquired when accessing and clearing ai_result",
        )
