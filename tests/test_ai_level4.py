...
from ai.level4 import and_or_search_move

class TestAndOrSearchTruncation(unittest.TestCase):
    def test_and_or_search_does_not_miss_critical_opponent_move(self):
        """
        Verify that and_or_search_move does not miss a critical opponent capture
        that is positioned late in the opponent's moves list due to arbitrary truncation.
        """
        board = Board(setup=False)
        board.turn = 'red' # Red's turn
        
        # Place Generals (with blocking Pawn at (6, 4) to prevent face-off check)
        board.matrix[0][4] = Piece('G', 'black', (0, 4), '將')
        board.matrix[9][4] = Piece('G', 'red', (9, 4), '帥')
        board.matrix[6][4] = Piece('P', 'red', (6, 4), '兵')
        
        # Place Red Advisors and Elephant to block General and Rook moves
        # to ensure Red has <= 10 total moves (preventing first-level truncation from skipping targets)
        board.matrix[9][3] = Piece('A', 'red', (9, 3), '仕')
        board.matrix[9][5] = Piece('A', 'red', (9, 5), '仕')
        board.matrix[9][2] = Piece('E', 'red', (9, 2), '相')
        ...
        # Run AND-OR Search
        best_move = and_or_search_move(board)
        ...
        self.assertEqual(best_move, ((9, 0), (9, 1)), "Should choose the safe capture at (9, 1) rather than the trap at (7, 0)")
