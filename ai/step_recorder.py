...
class BFSStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'id': 'n3', 'move': ..., 'depth': 1, 'score': None}
    queue: List[Dict[str, Any]] = field(default_factory=list)       # Hàng đợi FIFO: [{'id', 'move', 'depth'}]
    explored: List[Dict[str, Any]] = field(default_factory=list)    # Các node đã duyệt xong


@dataclass
class DFSStep(BaseStep):
    """DFS dùng Stack LIFO (đệ quy), đi sâu rồi backtrack"""
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'depth': 2, 'score': ...}
    stack: List[Dict[str, Any]] = field(default_factory=list)       # Stack đệ quy: nhánh đang đi sâu
...
class UCSStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'g_cost': 200, 'piece_captured': 'Xe'}
    frontier: List[Dict[str, Any]] = field(default_factory=list)    # Priority Queue sorted by g_cost tăng dần
    explored: List[Dict[str, Any]] = field(default_factory=list)
...
class GreedyStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'h': 900, 'piece': 'Xe'}
    candidates: List[Dict[str, Any]] = field(default_factory=list)  # Tất cả nước đi + h(n) của chúng


@dataclass
class AStarStep(BaseStep):
    """A* dùng f(n) = g(n) + h(n)"""
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'g': 200, 'h': 3500, 'f': 3700}
...
class IDAStarStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'g': ..., 'h': ..., 'f': ...}
...
class HillClimbStep(BaseStep):
    ...
    neighbors: List[Dict[str, Any]] = field(default_factory=list)   # [{'move': ..., 'score': ...}] tất cả hàng xóm
...
class SAStep(BaseStep):
    ...
    current_move: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'score': ...}
    candidate_move: Dict[str, Any] = field(default_factory=dict)    # {'move': ..., 'score': ...}
...
class BeamStep(BaseStep):
    ...
    all_candidates: List[Dict] = field(default_factory=list)        # Tất cả candidates trước khi cắt
    ...
    worst_case_scores: List[Dict] = field(default_factory=list)     # Score sau khi đối thủ phản công (minimax-like)
...
class OnlineStep(BaseStep):
    ...
    weights_before: Dict[str, int] = field(default_factory=dict)    # Trọng số trước khi điều chỉnh
    weights_after: Dict[str, int] = field(default_factory=dict)     # Trọng số sau khi điều chỉnh
    candidates: List[Dict] = field(default_factory=list)            # Nước đi đánh giá theo trọng số mới
...
class AndOrStep(BaseStep):
    ...
    and_responses: List[Dict] = field(default_factory=list)         # Tất cả phản công của đối thủ (AND)
...
class BeliefStep(BaseStep):
    ...
    belief_probs: Dict[str, float] = field(default_factory=dict)    # {'aggressive': 0.6, 'defensive': 0.2, 'positional': 0.2}
    utility_per_style: Dict[str, float] = field(default_factory=dict)  # {'aggressive': u1, 'defensive': u2, 'positional': u3}
...
class BacktrackStep(BaseStep):
    ...
    variables: Dict[str, int] = field(default_factory=dict)         # {from_pos: domain_size} tất cả quân
    chosen_variable: str = ""         # Quân được chọn (MRV = domain nhỏ nhất)
    domain: List[Dict] = field(default_factory=list)                # Các ô đích khả dĩ của quân được chọn
    best_assignment: Dict = field(default_factory=dict)             # Ô đích tốt nhất (score cao nhất)
...
class MinConflictStep(BaseStep):
    ...
    candidates: List[Dict] = field(default_factory=list)            # [{'move': ..., 'conflicts_after': ..., 'score': ...}]
...
class AC3Step(BaseStep):
    ...
    safe_moves: List[Dict] = field(default_factory=list)            # Nước an toàn (không bị quân rẻ ăn)
    pruned_moves: List[Dict] = field(default_factory=list)          # Nước bị lọc (unsafe: quân đắt đi vào ô bị quân rẻ bảo vệ)
    chosen_from_safe: Dict = field(default_factory=dict)            # Nước tốt nhất trong safe_moves
...
class MinimaxStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'depth': 2, 'is_max': True, 'value': 350}
    current_path: List[Dict] = field(default_factory=list)          # Đường đi từ root → node hiện tại (KHÔNG lưu toàn bộ cây)
    siblings_evaluated: List[Dict] = field(default_factory=list)    # Các nhánh cùng cấp đã duyệt xong + value
    best_so_far: Dict = field(default_factory=dict)                 # Nước tốt nhất tìm được đến hiện tại


@dataclass
class AlphaBetaStep(BaseStep):
    """Alpha-Beta: Minimax + cắt tỉa nhánh thừa"""
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'depth': ..., 'is_max': ..., 'value': ...}
    current_path: List[Dict] = field(default_factory=list)          # Đường đi từ root → node hiện tại
...
class ExpectimaxStep(BaseStep):
    ...
    current_node: Dict[str, Any] = field(default_factory=dict)      # {'move': ..., 'depth': ..., 'is_ai_turn': True}
    is_chance_node: bool = False      # True nếu là node CHANCE (lượt đối thủ)
    child_values: List[Dict] = field(default_factory=list)          # [{'move': ..., 'value': ...}] giá trị từng nhánh con
    best_value: float = 0.0           # Giá trị tốt nhất (nếu MAX node)
    expected_value: Optional[float] = None  # 0.7 * best + 0.3 * avg(others) (nếu CHANCE node)
